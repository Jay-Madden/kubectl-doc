...
func TestRenderFoldableHTML(t *testing.T) {
	...
	for _, expected := range []string{
		...
		".kdoc-details{align-self:start;background:var(--kdoc-details-bg);",
      ...
      applyFolds();`,
		...
		"--kdoc-ok",
		"kdoc-required-label",
		".kdoc-required-label{background:var(--kdoc-required-bg);",
		...
		".kdoc-detail-badge-required{background:var(--kdoc-required-bg);",
		".kdoc-detail-badge-optional{background:var(--kdoc-ok-bg);",
		"kdoc-detail-body",
		"kdoc-detail-grid",
		...
		`highlightedElements = [];`,
		".kdoc-filter-overlay{background:var(--kdoc-filter);border:1px solid var(--kdoc-filter-border);border-radius:6px;box-shadow:var(--kdoc-filter-shadow);",
